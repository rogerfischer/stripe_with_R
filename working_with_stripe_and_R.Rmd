...
```
In the CVS file itself the names "year and week", "lost", "new", "recurring“ are missing.
You can always add the row names by checking the documentation about [write.table](https://stat.ethz.ch/R-manual/R-devel/library/utils/html/write.table.html).
...
```
Every function gives you some information about your data frame respectively about your payments.csv data. With View(raw_data), you see all your data in tabular form. As you now see that there is too much data, we first do some cleanup. 
...
Now we will build our table that you can directly check up in R Studio's console
```r
...
6  12-48    0  12        14
```
you can install the [reshape2 package](http://cran.r-project.org/web/packages/reshape2/index.html) and the load it with library(reshape2). Just run this code chunk:
```r
...
Obviously this is just a first attempt to work with the Stripe data and I hope I can improve in any direction. Next up will be the week seen from its financial side. And then probably the first visualization of the data.
...
PS: The numbers in the examples are made up.
